    ...
    {!isLoading && !isError && visibleResults.length > 0 && (
      <div className="no-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        {visibleResults.map((product) => (
          <Link key={product.id} href={`/products/${product.id}`} className={`flex items-center gap-4 rounded-md p-3 hover:bg-[color:var(--app-surface-alt)] ${focusRingClass}`} onClick={onSelect}>
